...
{block name="content_body" append}
    ...
    <div class="breadcrumb">
        <span itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="breadcrumb__item{if $breadcrumbs|count == 0} breadcrumb__item--active{/if}">
          <a href="{url id="assets.overview.locale" parameters=["locale" => $locale]}?view={$view}&flatten={$flatten}&embed={$embed}&limit={$limit}&selected={$app.request->getQueryParameter('selected')}" itemprop="url">
        ...
        {foreach $breadcrumbs as $id => $name}
            <span itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="breadcrumb__item{if $name@last} breadcrumb__item--active{/if}">
              <a href="{url id="assets.folder.overview" parameters=["locale" => $locale, "folder" => $id]}?view={$view}&flatten={$flatten}&embed={$embed}&limit={$limit}&selected={$app.request->getQueryParameter('selected')}" itemprop="url">
