...
{if $table->hasRows() || $table->hasSearch() || $tableActions}
    ...
    <form id="{$tableForm->getId()}" action="{if isset($tableAction)}{$tableAction}{else}{$table->getFormUrl()}{/if}" method="POST" class="table spacer" role="form" data-confirm-messages="{$tableMessages}">
